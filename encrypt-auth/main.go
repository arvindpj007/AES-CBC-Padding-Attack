...
//Funciton that performs the AES-CBC encryption
func aescbcEncrypt(key, iv, message string) string {
	...
	for i := 0; i < int(len(message)/32); i++ {
		...
		block := hexToBytes(hexXOR(c[i], messagePart, 32))
...
func aescbcDecrypt(aesKey, hmacKey, cipher string) string {
	...
	// fmt.Println("plaintextAndHmacAndPadding: ", plaintextAndHmacAndPadding, len(plaintextAndHmacAndPadding))
	// Padding Check
	...
	if lengthOfPlainText-2*m < 0 {
		fmt.Println("ERROR: INVALID PADDING")
		os.Exit(1)
	}
	...
	if len(plaintextAndHmac)-64 < 0 {
		// fmt.Println("here: ", len(plaintextAndHmac))
		fmt.Println("ERROR: INVALID MAC")
		os.Exit(1)
	}
	...
	// fmt.Println("HMAC check: ", hmacCheck)
	if hmacCheck == false {
		fmt.Println("ERROR: INVALID MAC")
		os.Exit(1)
	}
...
func main() {
	...
	var aesKey string
	var hmac string
	var messageAndHmac string
	var padding string
	var messageAndHmacAndPadding string
	...
	mode, key, input, output = setupCLI()
	hmacKey = key[0:32]
	aesKey = key[32:64]
	...
	case "encrypt":
		...
		// Step 1. HMAC-SHA256 algorithm
		hmac = hmacSHA256(hmacKey, plaintext) //key from CLI, after splitting the key into 2
		// fmt.Println("hmac:", hmac, hexToBytes(hmac))
		// Step 2. M′ = M||T
		messageAndHmac = plaintext + hmac
		// fmt.Println(messageAndHmac)
		// Step 3. M′′ = M′||PS
		n := (len(messageAndHmac) / 2) % 16
		// fmt.Println("HMAC: ", hmac)
		setupPadding()
		padding = paddingMap[n]
		// fmt.Println("Padding: ", padding)
		messageAndHmacAndPadding = messageAndHmac + padding
		// fmt.Println("messageAndHmacAndPadding: ", messageAndHmacAndPadding, " ", len(messageAndHmacAndPadding)/2)
		// Step 4. AES-CBC
		iv := generateIV()
		// fmt.Println(iv)
		cipherText := aescbcEncrypt(aesKey, iv, messageAndHmacAndPadding) //key from CLI, after splitting the key into 2
		// fmt.Println("AA ", binToHex(cipherText), len(cipherText))         // store in cipher in output text
	...
	case "decrypt":
		...
		plaintext := aescbcDecrypt(aesKey, hmacKey, cipherText) //from CLI split keys
